...
| * | 匹配任意数量的字符。 |
| ？ | 匹配任意一个字符。 |
| [放] | 匹配指定集合中的任意字符。提示：集合由字符或范围组成。范围的格式为：字符 连字符，例如 AZ 或 0-9。集合支持的最小字符集为 [O-9a-zA-Z_]。 |
| [!set] 或 [^set] | 匹配不在指定集合中的任何字符。 |
| 一 | 抑制集合内字符 * []* ? !^ -' 的特殊句法意义，并精确匹配该字符。 |
...
| 快捷键 | 结果 |
| --- | --- |
| 哈* | 突出显示所有以 A 开头的网络，例如 AoO、AO1、A02。 |
| 氢+?? | 突出显示所有以 + 开头、O 后有两位数字或字符的网络，例如 +5V。 |
| HA?0 | 突出显示所有以 A 开头、以 0 结尾以及中间任意字符的网络，例如 Aoo 和 A10。 |
| H[CD]* | 突出显示所有以 C 或 D 开头的网络，例如 CLKIN、D00。 |
| H [!CD]* | 突出显示所有不以 C 或 D 开头的网络，例如 Aoo、GND。 |
...
2. 单击窗口标题栏中的 Pin  I。
 ...
 Pin 图片变为指向侧面。界面侧面会出现一个新栏。新栏出现在哪一侧取决于窗口的位置。例如，如果项目资源管理器位于用户界面左侧，当您从菜单中点击“自动隐藏”设置时，新栏就会出现在界面左侧。
...
> [!TIP]
为了最大化您的工作空间，请尝试将这些嵌入窗口设置为自动隐藏。按住 Ctrl 键并单击窗口标题栏上的 Pin ，原始窗口框架内的所有窗口都会自动隐藏。
