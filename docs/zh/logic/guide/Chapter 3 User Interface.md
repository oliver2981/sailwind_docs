...
4. 在“表格配置”区域中，指定 CIS 选项卡中显示的表格字段内容及其显示方式。有关更多信息，请参阅下表。
   ...
   | --- | --- |
   | 字段名称 | 列出左侧选定表中的所有字段。 |
   | 字段类型 | 从下拉列表中指定表字段所属的类型，其中：· Part_Type 为必填项，用于将数据加载到 CIS 选项卡中。此外，您还可以在 CIS 预览窗口中查看本地库中分配给该零件类型的原理图符号和 PCB 贴花。· Part_Number 为必填项，用于检查设计中的零件属性值是否与 CIS 中的一致。· Category 允许在 CiS 选项卡中按子类别分层显示表格结构。· 除“常规”字段外，所有字段类型都必须唯一。 |
...
3. 使用过滤器查找零件，然后单击“搜索”以激活过滤器。系统将显示符合搜索过滤器设置的零件。

   支持按字段名称和关键字进行过滤，但目前不支持通配符或表达式。

4. 选择一个零件并在预览窗口中检查其零件类型（CAE 贴花）和 PCB 贴花。
...
6. 在设计中选择零件，右键单击，然后点击“属性”弹出菜单项。在属性列表中，您可以看到通过“库配置”对话框中的“传输到设计”功能添加的属性。
...
3. 在“零件属性信息”区域中检查搜索结果，其中错误以红色突出显示。您还可以：
...
4. 通过以下任一方式从 CIS 更新不一致的原理图零件属性，这仅对发现“属性不相等”错误的零件有效。
