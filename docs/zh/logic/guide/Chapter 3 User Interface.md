...
会话日志 在状态选项卡中导航页面 过滤状态选项卡显示 在状态选项卡中搜索 打印会话日志消息 查看和打印报告 将会话日志保存到文件 清除会话日志显示
...
<table><thead><tr><th><br>颜色</th><th><br>意义</th></tr></thead><tbody><tr><td><br>红色的</td><td><br>错误</td></tr><tr><td><br>绿色的</td><td><br>警告</td></tr><tr><td><br>黑色的</td><td><br>消息</td></tr><tr><td><br>蓝色的</td><td><br>指向文件、网页和数据库对象的链接</td></tr></tbody></table>
...
<table><thead><tr><th><br>命令</th><th><br>描述</th></tr></thead><tbody><tr><td><br>后退</td><td><br>显示上一页。</td></tr><tr><td><br>向前</td><td><br>显示下一页。</td></tr></tbody></table>

表 4. 状态选项卡工具栏按钮（续）

<table><thead><tr><th><br>命令</th><th><br>描述</th></tr></thead><tbody><tr><td><br>停止</td><td><br>停止页面更新。</td></tr><tr><td><br>刷新</td><td><br>刷新报告和其他页面的显示。</td></tr><tr><td><br>家</td><td><br>返回会话日志。</td></tr></tbody></table>
...
<table><thead><tr><th><br>命令</th><th><br>描述</th></tr></thead><tbody><tr><td><br>错误</td><td><br>显示错误。</td></tr><tr><td><br>警告</td><td><br>显示警告。</td></tr><tr><td><br>信息</td><td><br>显示消息。</td></tr><tr><td><br>全部显示</td><td><br>显示所有项目（错误、警告和消息）。</td></tr></tbody></table>
...
2. 在库配置对话框中，按如下方式添加并连接到目标数据源：a. 单击 ODBC 配置。b. 在弹出窗口中添加要使用的 ODBC 数据源。c. 单击更新以使新添加的数据源处于活动状态并在 ODBC DSN 列表中可用。d. 从列表中选择目标数据库以建立连接并在数据库表区域中加载其表列表。
3. 通过选中“到 CIS”复选框来选择要使用的数据库表。

只有选定的内容才会在 CIS 选项卡中可用。

1. 在“表格配置”区域中，指定 CIS 选项卡中显示的表格字段内容及其显示方式。有关更多信息，请参阅下表。

表6. 表格配置说明

<table><thead><tr><th><br>姓名</th><th><br>描述</th></tr></thead><tbody><tr><td><br>字段名称</td><td><br>列出左侧选定表中的所有字段。</td></tr><tr><td><br>字段类型</td><td><br>从下拉列表中指定表字段所属的类型，其中：</td></tr></tbody></table>

<table><thead><tr><th><br>姓名</th><th><br>描述</th></tr></thead><tbody><tr><td></td><td><br>· Part_Type 为必填项，用于将数据加载到 CIS 选项卡中。此外，您还可以在 CIS 预览窗口中查看本地库中分配给该零件类型的原理图符号和 PCB 贴花。· Part_Number 为必填项，用于检查设计中的零件属性值是否与 CIS 中的一致。· Category 允许在 CiS 选项卡中按子类别分层显示表格结构。· 除“常规”字段外，所有字段类型都必须唯一。</td></tr><tr><td><br>字段别名</td><td><br>指定在 Cis 选项卡中显示的每个字段的表头。· 字段类型“Part_Type”和“Part_Number”对应的字段别名已默认定义，不允许修改。· 如果未设置任何内容，则将使用“字段名称”代替。</td></tr><tr><td><br>转移到设计</td><td><br>指定将字段名称添加到零件属性中。如果是，您可以在设计中检查零件属性，并在零件属性列表中查看该字段名称。提示</td></tr><tr><td><br>CIS 中的可见性</td><td><br>设置后，将改用“字段别名”。指定在 Cis 选项卡中显示“字段名称”。设置后，将改用“字段别名”。</td></tr><tr><td><br>钥匙</td><td><br>预订的</td></tr><tr><td><br>可浏览</td><td><br>指定在 Cis 选项卡中的字段内容中添加超链接，这些超链接通常链接到数据表和图纸等参考文件。</td></tr><tr><td><br>财产检查</td><td><br>指定要在零件管理器对话框中进行比较的属性，检查设计中的属性值是否与 CiS 中的属性值相同。</td></tr></tbody></table>
...
根据需要配置数据源后，您可以在设计中查看和使用 CIS 数据。为了高效地定位零件，SailWind Logic 提供了筛选功能。本节介绍如何使用 CIS 数据。

**程序**

1. 连接到目标数据源，并指定要显示的表字段，具体操作请参见“添加新配置”。2. 在 CIS 选项卡中，在左侧选择一个表，在右侧查看其部件信息。
2. 使用过滤器查找零件，然后单击“搜索”以激活过滤器。系统将显示符合搜索过滤器设置的零件。

支持按字段名称和关键字进行过滤，但目前不支持通配符或表达式。

1. 选择一个零件并在预览窗口中检查其零件类型（CAE 贴花）和 PCB 贴花。
2. 向设计中添加一个零件，如下所示：

a. 双击表格中的目标零件。该零件将附着并跟随光标移动。
b. 单击原理图以放置零件；另一个实例会自动附加到光标处。
c. 添加完部件后，按 Esc 键。

1. 在设计中选择零件，右键单击，然后点击“属性”弹出菜单项。在属性列表中，您可以看到通过“库配置”对话框中的“传输到设计”功能添加的属性。

#### 比较零件属性以进行一致性检查

使用零件管理器将设计中的零件属性与 CIS 中的属性进行比较，以检查一致性。对于不一致的属性，您可以使用多种选项从 CIS 进行更新。您还可以指定要比较的属性。
...
注意：比较是基于 CIS 属性 Part\_Number 进行的。
