...
导入过程会从库中获取所有元件类型和贴花，并将贴花堆叠在原点。然后，您可以使用“工具 \> “ 分散组件”菜单项来分散组件
...
2. 当提示您保存设计时，单击“否”。
3. 将 ascii.err 文件中列出的所有缺失元器件添加到您的库中，方法是将包含缺失元器件的库添加到库列表中，或者创建缺失的元件类型和贴花。（有关说明，请参阅《SailWind 布局指南》中的“将库添加到库列表”、“创建和修改元件类型”以及“创建和编辑 PCB 贴花”。）
...
• 在 SailWind Logic 中选择项目之前，应将第 712 页上的 SailWind Logic 选择过滤器设置为元件、门、网络或引脚。• 某些元件可能不存在于两个数据库中。未在 ECO 注册的元件和非电气元件可能已受到约束。有关详细信息，请参见“选项”对话框的“设计”类别。
...
• 在 SailWind Logic 中选择项目之前，应将第 712 页上的 SailWind Logic 选择过滤器设置为元件、门、网络或引脚。某些元件可能不存在于两个数据库中。未注册 ECO 的元件和非电气元件可能已受到约束。有关详细信息，请参见“选项”对话框的“设计类别”。
...
您可以向后注释元件、门、引脚、网络和属性的更改。有关详细信息，请参阅向后注释结果。
...
• SailWind Logic 在每个交换的引脚处创建一个页间连接符。

**添加到网络的引脚**

• 仅当引脚尚未连接到其他网络时，才可以添加该引脚。如果该引脚是门极引脚（门极符号上可见的端子引脚），则会创建一个页间连接符。

• 如果引脚已连接或引脚是已分配给网络的信号引脚，则会产生错误。

**引脚与网络断开**

• 如果引脚是门极引脚，则当其连接到连接点或页间连接符时，连接将被删除。如果连接到另一个门极引脚，则连接将被断开，并添加页间连接符。• 如果引脚未连接到相关网络，此命令将生成错误消息。
...
其中 refdes 是元件参考名称，pinnumber 是需要断开的引脚号，signame 是该引脚当前所属的网络。
