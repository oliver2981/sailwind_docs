...
反向注释支持多个设计活动层级。您可以使用反向注释交换设计更新以及属性、元件、门电路、网络和引脚的更改。结果取决于您正在更新的设计对象数据类型。

属性级后向注释 元件级后向注释 门级后向注释 网络级后向注释 引脚级后向注释
...
• 如果元件包含信号引脚，则这些引脚将包含在添加引脚功能中。反向注释目前不支持信号引脚，因此会生成错误消息。

**更换的元件**

• 如果更改的元件是多浇口元件，则所有浇口都将更新为新的元件类型。

• 如果设计或库中不存在新元件，或者门或引脚数不兼容，则会生成错误消息。

**已删除部分**

• 如果删除的元件是多浇口元件，则所有浇口都将被删除。
...
其中 refdes 是要删除的元件引用名称，parttype 是元件类型名称。如果待删除元件的所有引脚尚未与设计中的连接网络断开，则会报告错误。
