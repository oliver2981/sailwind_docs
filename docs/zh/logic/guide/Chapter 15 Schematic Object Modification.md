...
3. 从门贴花列表中选择一个门贴花名称，将选定门或元件的门贴花更改为预定义的替代贴花之一。
...
当将网表导入 SailWind Layout 时，使用分配给元件类型的默认贴花；
网表中未出现贴花分配。

1. 在“库中的替代品”列表中，从库中的当前元件类型定义中选择一个贴花。
2. 单击“分配”将贴花分配给元件。
...
使用“从库中获取贴花”对话框在库中搜索贴花。您可以使用筛选器快速缩小搜索参数，以找到所需的精确元件。

**程序**

1. 选择一个部件，右键单击，单击“属性”，然后单击“PCB 贴花”按钮。
2. 在 PCB 贴花分配对话框中，单击浏览按钮。
3. 在从库中获取贴花对话框中，使用过滤器将搜索结果限制为特定元件或项目名称的选定库（或多个库），或与第 105 页上的通配符或表达式匹配的名称。单击应用以搜索库并显示搜索结果。

> [!TIP]
搜索时，请遵循以下几点：

• 使用库下拉列表选择特定库目录或所有库设置。
• 在“项目”文本框中输入前缀和星号。例如，输入 ${\mathsf{D}}{\mathsf{I}}{\mathsf{P}}^{\star}$ 可查看所有以 DIP 开头的贴花。仅输入星号可查看所选库中的所有贴花。仅输入\* 可查看所选库中的所有元件或项目。
