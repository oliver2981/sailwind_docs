...
SailWind Logic 完全支持对设计对象的更改和编辑。这包括修改绘图对象、字段、元件、参考指示器、元件属性、标签、 Pin 和总线。您可以修改对象的单个实例，也可以选择更新库对象，以便将来的设计会话中可以使用任何修改。
...
• 如果您将元件类型更改为 Pin 数较少的元件类型，则连接到缺失 Pin 的连接不会被删除。这些连接将附加到自动生成的页间连接符上。所有断开的 Pin 都会通知您。
...
使用“元件信号 Pin ”对话框将任何未使用的 Pin 分配为附加信号 Pin 。创建元件类型并将其存储在库中时，将定义元件类型的标准电源 Pin 和接地 Pin 。

无法通过此对话框修改元件创建期间分配的信号 Pin 。请使用库管理器中的“为元件分配信号 Pin 名称”对话框。
...
另请参阅第 147 页上的“元件信息 -  Pin ”。

1. 在元件信号 Pin 对话框中，从未使用的 Pin 列表中选择一个 Pin 。
2. 单击“添加”将未使用的 Pin 添加到信号 Pin 列表。
3. 在信号 Pin 区域分配信号名称。
4. 要修改 Pin 的信号名称，请在信号 Pin 列表中选择该 Pin ，然后单击编辑。
5. 要将 Pin 移动到未使用的 Pin 列表，请在信号 Pin 列表中选择该 Pin ，然后单击删除。
6. 在“应用更新到”区域中设置更新设置。
...
• 在“ Pin 数”文本框中输入一个值，以进一步缩小搜索范围。如果所选元件已定义封装，则“ Pin 数”将灰显。如果“PCB 封装分配”对话框中的“无特定 PCB 封装”处于启用状态，则“ Pin 数”将启用。
...
1. 选择一个 Pin ，右键单击，然后单击“属性”。
2. 在“ Pin 属性”对话框中，单击“元件/门”按钮可更改元件或门的属性信息。另请参阅“修改元件”。
3. 单击“网络”按钮可修改网络名称和命名网络的属性。

> [!TIP]
修改网络名称或属性
$$
89$命名网络，使用查询模式按钮选择一个 Pin 附近的网络，将其从
$$
89$ 命名网络。另请参阅修改网络。4. 单击字体按钮打开 Pin 标签字体对话框，您可以在其中设置字体首选项。另请参阅修改 Pin 标签字体。5. 单击确定。## 修改 Pin 标签字体为了提高原理图的可读性，您可以使用 Pin 标签字体对话框更改 Pin 标签的字体。\*\*步骤\*\* 1. 选择一个 Pin 标签，右键单击，然后单击属性。2. 在 Pin 标签字体对话框的 Pin 区域中，在线宽度框中输入笔触字体的线宽。3. 对于笔触字体或系统字体，在大小框中输入大小（笔触字体以密耳为单位，系统字体以磅为单位）。!Tip 输入介于 10 到 1000 密耳之间的笔触字体大小或介于 1 到 72 磅之间的系统字体大小。4. 对于系统字体，选择要使用的字体。如果需要，您还可以单击字体样式：B 表示粗体、I 表示斜体或 U 表示带下划线。5. 单击确定。## 修改网络要重命名网络或编辑其属性和值，可以使用网络属性对话框访问网络特定属性。\*\*步骤\*\*1. 选择一个网络，右键单击，然后单击属性。!Tip 选择它进入或退出元件的连接段。选择连接的任何其他段将不允许添加网络名称标签。2. 在网络名称框中键入新名称或从列表中选择现有名称。!Tip 使用网络名称时，请注意以下事项：• 如果从列表中选择一个名称，则会显示消息“网络已存在 - 可以合并网络（Y/N）吗？”。单击“是”合并网络，或单击“否”输入其他网络名称。• 如果删除所有可见的网络名称标签，则网络和所有子网将保留该网络名称。网络名称不会恢复为生成的
