...
创建、管理和编辑库元件的相关流程包含多个细节层次。您可以使用贴图编辑器创建 CAE 贴图（原理图符号），包括编辑门、端子、引脚、引脚号、序列号和属性标签，然后使用元件编辑器将数据合并到元件类型中。

## 元件编辑器操作

使用元件编辑器创建和修改标准原理图符号（例如门电路、电阻器、电容器、连接器等）以及特殊符号（例如接地/电源和页外引用）。您还可以使用元件编辑器创建新的 CAE 封装、引脚封装，并分配 PCB 封装（即元件在 PCB 设计系统中的物理表示）。
...
| 元素 | 描述 |
| --- | --- |
| 元件类型 | 元件的电气信息。这包括逻辑系列、CAE 贴图分配、元件名称、连接引脚、门交换信息、属性等。另请参阅修改元件的电气信息。 |
| 封装 | 门。另请参阅创建新的 CAE 封装。 |
| PCB 封装 | 在 PCB 设计程序中绘制元件时的图形表示。通常称为封装。分配 PCB 封装是可选的，仅在将网表传递给 SailWind Layout 时才需要。 |

• 更改和更新库元件创建单门元件创建多门元件从现有元件创建新元件创建新连接器创建新 CAE 封装创建新引脚封装特殊原理图符号
...
您可以使用“选择过滤器”对话框控制可选择的对象。这可以快速选择元件、门、网络、引脚、总线、连接、二维线、文本和其他设计对象。
...
您可以通过调整现有元件来创建新元件。如果需要添加或删除引脚，请复制现有的 CAE 贴图。如果在未复制现有 CAE 贴图的情况下修改现有 CAE 贴图，则所有使用该贴图的元件都将受到影响。
...
1. 单击工具 \> 元件编辑器菜单项。
2. 在元件编辑器的工具栏上，单击“编辑电气”按钮。这将显示第 134 页的“元件信息”对话框。
3. 在“元件信息”对话框中，选择“PCB 封装”选项卡以分配 PCB 封装。分配的 PCB 封装的引脚数必须等于或大于电气描述中的数量。
4. 选择门选项卡。
5. 为元件中的每个浇口单击一次“添加”。浇口按字母数字顺序列在“浇口”多列列表框中。6. 双击某个浇口对应的“CAE 贴图”列，然后选择“浏览 (…)”按钮。此时将出现“将贴图分配给浇口”对话框。
6. 要过滤封装，请在过滤器框中键入第 105 页上的通配符或表达式，然后单击应用。
7. 单击“分配”。完成大门封装分配后，单击“确定”。
8. （可选）为元件分配浇口交换信息。有关更多信息，请参阅第 144 页的“浇口”选项卡主题。
9. 对其他门重复步骤 5 至 8。
10. 点击“引脚”选项卡，使用“引脚数”来指示器件中的引脚数。输入整个封装（包括信号引脚）的引脚数，而不是单个门电路的引脚数。
11. 在“常规”选项卡上，从列表中选择“逻辑系列”和“前缀”，并根据需要选择其他电气信息。另请参见修改元件的电气信息。
12. 单击“确定”。

**结果**

元件编辑器现在会显示已分配的 CAE 封装轮廓。请注意，此时不会显示引脚信息。
...
4. 单击封装编辑工具栏按钮。
5. 单击 CAE 封装向导按钮。
6. 要设置从端子连接点到封装轮廓的水平和垂直距离，请键入一个值或使用箭头在“引脚长度”区域中指示引脚长度。
...
1. 要设置封装轮廓的最小高度，请键入一个值或使用“最小高度”框中的箭头。

> [!TIP]
如果输入的值大于容纳输入或输出引脚数量所需的值，则会在封装底部添加空间。

1. 在左侧引脚区域中，从引脚封装列表中选择元件左侧或输入侧的引脚封装，然后在引脚数框中键入引脚数或使用箭头指示引脚数。
2. 在右侧引脚区域中，从引脚封装列表中选择元件右侧或输出侧的引脚封装，然后在引脚数框中键入引脚数或使用箭头指示引脚数。
3. 在“上部引脚”区域中，选择用于上部引脚的引脚封装，然后在“引脚数”框中键入引脚数或使用箭头指示引脚数。
4. 在“下部引脚”区域中，选择用于下部引脚的引脚封装，然后在“引脚数”框中键入引脚数或使用箭头指示引脚数。
5. 单击“确定”。

**结果**

封装向导创建一个正确大小的矩形，添加引脚，并为序列号、引脚号、引脚类型和交换信息创建占位符。

> [!TIP]
：使用“元件编辑器”的“绘图”工具栏修改封装向导创建的基本信息。您可以添加其他端子、更改引脚封装或更改引脚序列号。

### 手动构造新元件

您可以通过创建 2D 线体形状、添加端子、分配引脚编号和引脚名称来手动构建新元件以用于您的设计。
...
2. 在文件菜单上，选择返回元件，然后在出现将更改保留到门消息时单击是。
...
1. 手动为元件构建新的 CAE 封装或使用 SailWind-Logic 的 Decal Wizard 自动元件创建工具。

• 使用第 116 页的封装向导创建轮廓并添加端子。（可选）使用元件编辑器符号编辑工具栏添加其他端子、更改引脚封装或更改引脚序列号。
...
在以下步骤中，您将先创建 CAE 贴图，然后再输入电气信息。这会导致元件类型和 CAE 贴图共享相同的名称。如果您想使用现有的 CAE 贴图而不是创建新的贴图，请先指定电气信息。
...
1. 手动为元件构造新的 CAE 贴图（第 117 页）或使用贴图向导中的自动元件创建工具（第 116 页）构建元件。
2. 在元件编辑器中，如果您在步骤 4 中使用封装向导创建了 CAE 封装，请使用封装编辑工具栏修改封装向导创建的引脚编号，并添加引脚名称、门交换信息等。
3. 在文件菜单上，选择完成。
4. 为元件分配电气信息。

a. 在元件编辑器工具栏上，单击“编辑电气”按钮。将显示第 134 页上的“元件的元件信息”对话框。
b. 设置引脚数以指示元件中的引脚总数。如果元件中存在在上述步骤中创建 CAE 贴图时未添加的信号引脚，则必须设置此引脚数。（可选）单击“贴图”选项卡以分配 PCB 贴图。分配的 PCB 贴图的引脚数必须等于或大于电气描述。
c. 在“常规”选项卡上，从列表中选择一个逻辑系列。另请参阅查看和设置常规元件信息。（可选）选择“引脚”选项卡并分配电源和接地信号信息。
d. 选择“属性”选项卡并分配物料清单和元件清单信息。
e. 单击“确定”。元件编辑器将显示 CAE 贴图。
...
终端由一个引脚封装和一系列定义终端编号、交换数据等的文本字符串组成。

**程序**

1. 在封装编辑工具栏上，单击添加终端按钮。
2. 在“引脚封装浏览”对话框中，选择一个封装，然后单击“确定”。引脚封装将附加到光标上。
...
您可以双击图钉来跳过一个数字。
如果需要，请使用元件编辑器中编辑工具栏中的“更改引脚号”按钮来更改引脚号。
完成后，右键单击并单击取消。

### 设置引脚名称

使用“设置引脚名称”命令可添加或更改多个引脚的名称。引脚名称标记了引脚的功能，例如 CLK、DATA0 等。每次选择引脚时，名称后缀都会递增。此命令不检查重复的命名约定。

在文本前面加上反斜杠字符 ()，以在文本字符上添加横线。

**程序**

1. 在封装编辑工具栏上，单击设置引脚名称按钮。
2. 在信息窗口中输入引脚名称。
3. 单击“确定”。
4. 选择每个引脚来分配名称。

每次单击引脚时，引脚名称都会递增，这样您就可以按顺序为元件分配引脚名称（例如，D0、D1、D2，依此类推）。要跳过序列中的特定引脚名称，请选择相同的引脚，直到出现所需的引脚名称，然后继续选择其他引脚。

1. 完成后，右键单击并单击取消。
2. 使用元件编辑器封装编辑工具栏中的更改引脚名称按钮来更改单个引脚的名称。

### 设置 Pin 类型

使用“设置引脚类型”来更改终端所代表的引脚类型，例如负载、源等等。
执行此任务时，您可以选择动词模式或宾语模式。

使用动词模式设置引脚类型 使用对象模式设置引脚类型

#### 使用动词模式设置 Pin 类型

您可以使用动词模式设置引脚类型。启动命令，然后选择要应用该命令的一个或多个对象。

**程序**

1. 在封装编辑工具栏上，单击“设置引脚类型”按钮。此时将显示“引脚类型”信息窗口。
2. 从下拉列表框中选择引脚类型表 18 列出了可用的引脚类型及其对应的字母。
3. 选择要应用引脚类型信息的引脚。与引脚关联的文本字符串将被修改，以反映新的引脚类型。
4. 如有必要，重复步骤 2 和 3。
5. 右键单击并单击取消弹出菜单项以退出动词模式。

#### 使用对象模式设置引脚类型

您可以使用对象模式设置引脚类型。选择一个或多个对象，然后选择要对其执行操作的命令。

**程序**

1. 选择要应用引脚类型信息的封装引脚。
2. 在封装编辑工具栏上，单击“设置引脚类型”按钮。此时将显示“引脚类型”信息窗口。
3. 从下拉列表框中选择引脚类型。表 18 列出了可用的引脚类型及其对应的字母。
4. 单击“确定”。
...
> [!TIP]
要按升序为多个引脚分配编号，请使用元件编辑器中编辑工具栏上的“设置引脚编号”按钮。
...
您可以使用对象模式更改引脚编号。选择一个或多个对象，然后选择要对其执行操作的命令。

**程序**

1. 选择封装端子。
2. 在封装编辑工具栏上，单击更改引脚编号按钮。
3. 在信息窗口中输入新号码。
...
> [!TIP]
要按升序为多个引脚分配名称，请使用元件编辑器中编辑工具栏上的“设置引脚名称”按钮。
...
1. 选择封装端子。
2. 在封装编辑工具栏上，单击更改引脚名称按钮。
3. 在信息窗口中输入新名称。
4. 单击“确定”以更新选定的终端。

### 更换别针封装

您可以为原理图符号中现有的端子选择不同的引脚封装。此功能在定义连接器引脚用途或重新定义库中的符号时尤其有用。
...
1. 在封装编辑工具栏上，单击“更改引脚封装”按钮。引脚封装浏览对话框
出现方框。
...
使用元件编辑器封装编辑工具栏中的“更改序列号”可更改元件中引脚的顺序。引脚序列号用于为备用 CAE 封装中的引脚设置相应的编号。当您使用“设置引脚号”分配引脚号或使用“更改引脚号”更改引脚号时，所有备用封装都会分配相同的编号。
...
属性标签是 CAE 贴图内属性的占位符。当您向元件添加属性时，该属性将被放置在此预留位置。SailWind Logic 允许您创建无限数量的属性标签。
...
2. 要更改封装编辑器中选定的所有端子的引脚封装，请单击“更改引脚封装”。
...
您可以在元件编辑器中查看和修改 CAE 封装的引脚信息。如果您的符号要求使用 JEDEC 标准，您还可以指定 JEDEC 引脚编号。

**程序**

1. 在元件编辑器中，打开元件类型，然后单击“编辑图形”按钮。
2. 在“选择浇口封装”对话框中，从“浇口”列表中选择一个浇口，或从工作区中选择一个浇口。封装将显示在 CAE 封装编辑器中。
...
如果修改与某个元件类型关联的封装，请使用“将元件和浇口封装另存为”对话框重命名该封装。修改封装后，您可以使用其他名称保存它。
...
您可以使用元件编辑器来处理元件信息。元件信息包含电气和属性信息，用于表示元件的物理特性，而非 CAE 贴图中包含的图形表示。
...
使用“元件信息”对话框可修改元件的电气信息。电气信息包括元件统计信息、逻辑系列、PCB 封装分配、CAE 封装分配、门电路和信号引脚、门电路交换信息以及其他各种属性。
...
• 将门分配给元件 - 门选项卡 元件信息 - 引脚选项卡 管理属性 - 属性选项卡

元件信息 - 引脚映射选项卡 • 为连接器符号分配备用逻辑封装 - 连接器选项卡
...
| 类别 | 描述 |
| --- | --- |
| 引脚数 | 显示元件中的引脚总数。包括栅极引脚、信号引脚和未使用的引脚。如果为多个封装指定了不同的引脚数，则会显示从最小到最大的封装引脚数范围。 |
...
您可以设置一般元件信息，例如逻辑到物理引脚映射、特殊用途元件定义和元件的 ECO 注册。

**程序**

1. 在元件编辑器中，单击编辑电气按钮，然后在元件的元件信息对话框中，单击常规选项卡。2. 选中“定义元件类型引脚号到 PCB 封装的映射”复选框以激活引脚映射选项卡，您可以在其中将逻辑引脚号映射到不同的物理引脚号。

**限制：**

请注意以下限制：

• 向元件类型添加一个或多个字母数字封装后，复选框将不可用。移除已分配的字母数字封装即可启用复选框。即使分配了数字封装，复选框也会变为可用。但是，您仍然需要从列表中移除字母数字封装才能使元件有效。• 必须分配封装才能使用“引脚映射”选项卡。• 只有带有连续数字引脚编号的封装才可用于引脚映射。
...
• 当元件已有门或引脚分配时，此复选框不可用。
• 选中“连接器”复选框时，“门封装”选项卡不可用。
• 一些不适用于连接器元件的引脚选项卡控件被禁用。
...
在为元件分配门信息、信号引脚名称或引脚映射之前，必须分配封装。
封装还指定了元件中的针脚数量。
...
0 提示：删除“引脚数”框中的所有数字即可显示所有封装。此框始终可用作过滤器，以便分配不同引脚数的封装。

c. 单击“仅显示引脚号与元件类型匹配的封装”复选框，以过滤掉引脚号与“引脚”选项卡上现有门和信号引脚不匹配的封装，或与“引脚映射”选项卡上物理引脚号不匹配的封装。
...
• 您必须为“引脚”选项卡上定义的所有门引脚和信号引脚分配具有足够引脚的封装。• 只有具有连续数字引脚编号的封装才可用于引脚映射。

1. 要为元件分配其他封装，请重复上面的步骤 4 和 5。
2. 在“元件信息”对话框打开时，您可以检查元件中是否存在缺失或不一致的信息。单击“检查元件”按钮。

当您退出选项卡时，软件会检查分配的封装（无论您是否单击“检查元件”按钮），以确保它们包含在“引脚”选项卡中定义的所有门和信号引脚的物理引脚号。
...
门封装和替代门和引脚交换信息将门分配给元件

#### 登机口封装和替代品

对于每个门，您可以输入 CAE 贴图名称，该名称是用于在原理图中显示元件的逻辑符号的名称。备用贴图分配必须具有相同数量的引脚。每个门可以定义一个主贴图和三个备用贴图。在 SailWind Layout 中，分配 CAE 贴图是可选的，但在 SailWind Logic 中是必需的。

#### 门和引脚交换信息

如果某个元件至少分配了一个封装，您可以输入或修改其浇口信息。这包括启用或禁用元件内部或类似元件之间的浇口交换。这些信息可让 SailWind Logic 了解在放置后可以使用哪些浇口来最小化连接长度。

要进行浇口交换，请在“浇口”选项卡上为浇口分配一个编号。编号相同的浇口可以在元件内部交换，也可以与其他类似元件交换。数值一 (1) 表示该浇口可与原理图数据库中相同元件类型的浇口交换。如果元件包含多种可交换浇口，则用数字 2 标识第二种类型，用数字 3 标识第三种类型，依此类推。数字 0 表示不能进行交换。
...
2. 要为元件添加浇口，请点击“添加”。该浇口将显示在“浇口”列表中。第一个浇口自动指定为 A，第二个浇口指定为 B，依此类推。选择一个浇口后，“引脚”列表将显示所选浇口的引脚数量。
...
“为浇口指定封装”对话框用于为元件中的每个浇口指定封装。某些元件可能包含多个不同类型的浇口，因此您可以为每个浇口指定单独的特定封装。
...
• 您最多可以指定四个封装。指定的封装必须具有相同的引脚数。第一个封装是默认封装，即在原理图中添加元件时使用的封装。
...
您可以使用多种方法将引脚添加到元件。您可以通过分配封装自动添加所有引脚。您可以向元件添加单个引脚、添加一系列引脚以及从数据库粘贴引脚；或者，您可以使用逗号分隔值 (CSV) 文件导入引脚。
...
您可以使用“元件信息”对话框中的“引脚”选项卡将单个引脚添加到元件。

**程序**

1. 打开元件类型，然后在工具栏上单击“编辑电气”按钮。
2. 在元件信息对话框的引脚选项卡上，单击添加引脚按钮。
...
您可以使用“元件信息”对话框中的“引脚”选项卡向元件添加一系列引脚。

**程序**

1. 打开元件类型，在工具栏上单击“编辑电气”按钮，然后单击“元件信息”对话框中的“引脚”选项卡。
2. 在元件信息对话框的引脚选项卡上，单击添加引脚按钮。
3. 在“添加引脚”对话框中，在“引脚数”框中键入要添加的引脚数。
0 提示 该元件的总引脚数不能超过 32,767。
4. 在“起始引脚号”区域的“前缀”和/或“后缀”框中输入值。基于您输入的引脚号预览将显示在框下方。

> [!TIP]
输入密码时，请遵循以下事项：

• 任一框中均可使用字母和数字值。例如，A1 或 1A。如果您输入的是字母数字，而封装使用数字，则必须使用“引脚映射”选项卡将字母数字映射到封装上。• 对于单个数字，请使用“前缀”或“后缀”框，并清空另一个框。
...
注意：引脚编号必须与 PCB 封装相匹配。例如，字母数字对应字母数字。

> [!TIP]
引脚编号可以是数字或字母数字。在 PADS2007 之前的版本中，字母数字引脚编号在 PCB 封装上不合法，但会覆盖数字封装编号，并存储在“元件类型”中。您可以继续保留数字 PCB 封装，并使用“引脚映射”选项卡将不同的引脚编号叠加到数字 PCB 封装引脚编号上。请参阅“元件信息 - 引脚映射”。
...
1. 单击“序列 (Seq.)”单元格并输入门序列号。序列号决定了 CAE 门引脚到 PCB 封装引脚的映射。该序列会自动与备用 CAE 封装共享。例如，序列号显示了引脚号在 CAE 门封装上的显示方式；因此，在门 A 中，序列号 1 可能是引脚 1，但在门 B 中，序列号 1 可能是引脚 4。
...
为隐式引脚（即原理图中的任何门电路上均未显示的引脚）分配信号名称。通常，接地引脚和电源引脚是唯一的隐式引脚。您无需使用信号引脚。您可以为门电路添加电源引脚和接地引脚，或者为电源引脚和/或接地引脚创建单独的门电路。对于 SailWind Logic 附带库中的元件，标准接地信号名称为 GND。标准电源信号名称为 $+5V.$

#### 分配未使用的引脚

您可以将某个引脚指定为未使用引脚。未使用引脚是指在 PCB 贴图上定义但在元件类型中不具备电气功能的引脚。未使用引脚的信息不会保存在元件类型中，而是根据已分配的门极引脚和信号引脚数量以及已分配的 PCB 贴图上的引脚数量自动推导出来的。
...
重新编号的引脚显示在“元件信息”对话框的“引脚”选项卡上。

#### 删除 Pin

如果需要修改元件，您可以在“引脚”选项卡上的“引脚”表中删除引脚。
...
• 在“常规”选项卡上，选中“定义元件类型引脚号到 PCB 封装的映射”复选框，以启用“引脚映射”选项卡。在“PCB 封装”选项卡上，指定一个带有连续数字引脚号的封装，即可使用“引脚映射”选项卡。封装决定了元件中的引脚数。
...
您可以使用 Pin Mapping 选项卡将字母数字引脚编号映射到数字封装。

**先决条件**

• 在“元件信息”对话框的“常规”选项卡中，选中“定义元件类型引脚号到 PCB 封装的映射”复选框，以启用“引脚映射”选项卡。在“PCB 封装”选项卡中，指定一个具有连续数字引脚号的封装，即可使用“引脚映射”选项卡。该封装决定了元件中的引脚数。

**程序**

1. 单击“元件信息”对话框中的“引脚映射”选项卡。在预览窗口上方的封装列表中，选择要映射字母数字引脚的指定封装。
2. 使用以下方法之一映射引脚：

• 在“未映射的引脚”列表中，选择一个或多个字母数字。如果您要映射的是连续的列表，请选择“元件类型”列中的一项或首行。点击“映射”。
• 在“元件类型”列中选择一个单元格，然后单击“编辑”按钮，或者直接双击该单元格。在“未映射引脚”列表中选择一个字母数字，然后在封装预览窗口中双击该引脚，即可将该字母数字映射到该引脚。“未映射引脚”列表中的下一行将成为下一个选定的要映射的字母数字。
...
您可以使用 Pin Mapping 选项卡取消封装上的引脚映射。
...
使用“将元件和浇口封装另存为”对话框将元件类型保存到库中。您还可以在保存过程中重命名关联的封装。如果您更改封装中的引脚数或其他信息，这可以防止使用相同封装的其他元件受到影响。

> [!TIP]

如果减少关联封装中的引脚数，则“元件属性”对话框的“更改类型”选项将不会使用库中的修改版本更新元件的原理图副本。
...
连接器与其他元件的不同之处在于，连接器通常显示为多个单独的引脚，而不是单个完整的元件。在原理图上使用连接器时，引脚编号（而不是参考名称）会递增，例如 P1-1、P1-2、P1-3，而不是 P1、P2、P3。
...
4. 在“引脚”选项卡上，添加引脚。有关更多信息，请参阅元件信息 - 引脚。
5. 选择连接器选项卡。
...
引脚封装用于表示元件上的端子引脚。您可以复制现有引脚封装并进行修改，也可以创建新的引脚封装。

从现有别针封装创建新别针 创建新别针封装
...
3. 从“引脚”区域中选择一个现有的引脚封装。高亮显示的封装将显示在“图片”区域中。
4. 单击“确定”。
...
当您创建新的引脚封装时，SailWind Logic 会自动创建与引脚信息相关的必要文本字符串。您只需指定代表端子引脚的二维线条，然后重新定位文本字符串即可。
...
• #E 是引脚号的占位符。
• PNAME 是引脚名称的占位符。NETNAME 是网络名称的占位符。
• #0:TYP $\cdot=\cup$ SWP $\scriptstyle=0$ 是序列号、引脚类型和交换类别的占位符。
...
提示：要控制在引脚封装列表中显示哪些封装，请使用引脚列表管理器
