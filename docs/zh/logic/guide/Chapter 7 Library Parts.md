...
使用元件编辑器创建和修改标准原理图符号（例如门电路、电阻器、电容器、连接器等）以及特殊符号（例如接地/电源和页外引用）。您还可以使用元件编辑器创建新的 CAE 贴花、引脚贴花，并分配 PCB 贴花（即元件在 PCB 设计系统中的物理表示）。
...
| 贴花 | 门。另请参阅创建新的 CAE 贴花。 |
| PCB 贴花 | 在 PCB 设计程序中绘制元件时的图形表示。通常称为封装。分配 PCB 贴花是可选的，仅在将网表传递给 SailWind Layout 时才需要。 |
...
> [!INFO]
复制元件的过程与复制 CAE 贴花的过程相同，只是您从“选择编辑项类型”对话框中选择“元件类型”，而不是选择“CAE 贴花”。

• 更改 CAE 贴花和电气信息，然后将其保存为新元件。
• 您可以重命名通过元件类型访问的已修改贴花。

另请参阅保存元件类型。

## 将元件/贴花保存到库中

在元件编辑器中，您可以将元件/贴花保存到库中。将元件添加到库后，即可将其放入设计中。
...
使用元件编辑器，您可以通过添加门和定义逻辑系列来构建新的 CAE 贴花。

**程序**

1. 单击工具 \> 元件编辑器菜单项。
2. 在元件编辑器的工具栏上，单击“编辑电气”按钮。这将显示第 134 页的“元件信息”对话框。
3. 在“元件信息”对话框中，选择“PCB 贴花”选项卡以分配 PCB 贴花。分配的 PCB 贴花的引脚数必须等于或大于电气描述中的数量。
4. 选择门选项卡。
...
元件编辑器现在会显示已分配的 CAE 贴花轮廓。请注意，此时不会显示引脚信息。
...
1. 在左侧引脚区域中，从引脚贴花列表中选择元件左侧或输入侧的引脚贴花，然后在引脚数框中键入引脚数或使用箭头指示引脚数。
2. 在右侧引脚区域中，从引脚贴花列表中选择元件右侧或输出侧的引脚贴花，然后在引脚数框中键入引脚数或使用箭头指示引脚数。
...
> [!TIP]
：使用“元件编辑器”的“绘图”工具栏修改贴花向导创建的基本信息。您可以添加其他端子、更改引脚贴花或更改引脚序列号。
...
创建新元件。
5. 在元件编辑器工具栏上，单击贴花编辑工具栏按钮以打开符号编辑工具栏。
...
1. 手动为元件构建新的 CAE 贴花或使用 SailWind-Logic 的 Decal Wizard 自动元件创建工具。

• 使用第 116 页的贴花向导创建轮廓并添加端子。（可选）使用元件编辑器符号编辑工具栏添加其他端子、更改引脚贴花或更改引脚序列号。
...
1. 单击工具 \> 元件编辑器菜单项。
2. 在元件编辑器中，单击编辑 \> CAE 贴花编辑器菜单项，或在元件编辑器工具栏上单击编辑图形按钮。
3. 在“选择门贴花”对话框中，输入贴花的名称，然后在提示为新元件创建贴花时单击“确定”。
...
2. 在元件编辑器中，如果您在步骤 4 中使用贴花向导创建了 CAE 贴花，请使用贴花编辑工具栏修改贴花向导创建的引脚编号，并添加引脚名称、门交换信息等。
3. 在文件菜单上，选择完成。
4. 为元件分配电气信息。
...
1. 完成后，右键单击并单击取消。
2. 使用元件编辑器贴花编辑工具栏中的更改引脚名称按钮来更改单个引脚的名称。
...
1. 在贴花编辑工具栏上，单击“设置引脚类型”按钮。此时将显示“引脚类型”信息窗口。
...
1. 选择要应用引脚类型信息的贴花引脚。
2. 在贴花编辑工具栏上，单击“设置引脚类型”按钮。此时将显示“引脚类型”信息窗口。
...
1. 在贴花编辑工具栏上，单击“更改引脚贴花”按钮。引脚贴花浏览对话框
出现方框。
...
使用元件编辑器贴花编辑工具栏中的“更改序列号”可更改元件中引脚的顺序。引脚序列号用于为备用 CAE 贴花中的引脚设置相应的编号。当您使用“设置引脚号”分配引脚号或使用“更改引脚号”更改引脚号时，所有备用贴花都会分配相同的编号。
...
• 属性标签不会显示在原理图编辑器或元件编辑器中。您必须查看贴花才能查看属性标签。创建或编辑贴花时，使用“显示颜色”可控制属性标签的显示。另请参阅“设置显示颜色”。
...
在元件编辑器中，使用“设置”菜单\> “ 设置原点”为 CAE 贴花指定新的原点。在 SailWind Logic 中添加元件时，插入点由贴花的原点决定。您可以在创建或修改贴花时执行此任务。
...
您可以在元件编辑器中查看和修改 CAE 贴花的引脚信息。如果您的符号要求使用 JEDEC 标准，您还可以指定 JEDEC 引脚编号。
...
如果修改与某个元件类型关联的贴花，请使用“将元件和浇口贴花另存为”对话框重命名该贴花。修改贴花后，您可以使用其他名称保存它。

**程序**

1. 单击文件 \> 返回元件菜单项，然后在出现“返回元件级别 - 保留对门的更改？”提示时单击“是”。
2. 单击“文件 \> 另存为菜单项。将出现“将元件和门贴花另存为”对话框。
3. 在“门贴花名称”多列列表框中选择贴花，然后单击“编辑”。
4. 为贴花输入一个新名称，然后单击“确定”。
5. 对于“元件类型项存在。覆盖项？”提示，单击“是”，以使用包含修改后的贴花的元件替换当前库元件。
...
使用“元件信息”对话框可修改元件的电气信息。电气信息包括元件统计信息、逻辑系列、PCB 贴花分配、CAE 贴花分配、门电路和信号引脚、门电路交换信息以及其他各种属性。

在为元件分配 CAE 贴花之前，您必须识别电气或元件类型信息。
...
元件信息 - 引脚映射选项卡 • 为连接器符号分配备用逻辑贴花 - 连接器选项卡
...
| 类别 | 描述 |
| --- | --- |
| 引脚数 | 显示元件中的引脚总数。包括栅极引脚、信号引脚和未使用的引脚。如果为多个贴花指定了不同的引脚数，则会显示从最小到最大的贴花引脚数范围。 |
| 贴花 | 显示贴花的名称，如在 PCB 贴花选项卡上选择的。 |
| 门数 | 显示元件中存在多少个浇口。 |
| 信号引脚数 | 显示元件中的信号引脚数。 |
...
1. 在元件编辑器中，单击编辑电气按钮，然后在元件的元件信息对话框中，单击常规选项卡。2. 选中“定义元件类型引脚号到 PCB 贴花的映射”复选框以激活引脚映射选项卡，您可以在其中将逻辑引脚号映射到不同的物理引脚号。

**限制：**

请注意以下限制：

• 向元件类型添加一个或多个字母数字贴花后，复选框将不可用。移除已分配的字母数字贴花即可启用复选框。即使分配了数字贴花，复选框也会变为可用。但是，您仍然需要从列表中移除字母数字贴花才能使元件有效。• 必须分配贴花才能使用“引脚映射”选项卡。• 只有带有连续数字引脚编号的贴花才可用于引脚映射。
...
要将贴花或封装分配给库元件，请使用“元件信息”对话框中的“PCB 贴花”选项卡。
在为元件分配门信息、信号引脚名称或引脚映射之前，必须分配贴花。
贴花还指定了元件中的针脚数量。
...
在某些情况下，您可能需要为一个元件定义多个 PCB 贴花。这样，一个符号就可以代表存在于不同物理封装中的不同版本的元件。您最多可以为一个元件分配 16 个 PCB 贴花。

**程序**

1. 打开元件类型。
2. 在元件编辑器的工具栏上，单击“编辑电气”按钮，然后在元件信息对话框中，单击“PCB 贴花”选项卡。
...
c. 单击“仅显示引脚号与元件类型匹配的贴花”复选框，以过滤掉引脚号与“引脚”选项卡上现有门和信号引脚不匹配的贴花，或与“引脚映射”选项卡上物理引脚号不匹配的贴花。

1. 从“未指定的贴花”列表框中选择一个贴花名称。
2. 单击“分配”。选定的贴花将分配给元件，并移动到“已分配贴花”列表框中。

> [!TIP]
作为快捷方式，双击贴花名称即可将其分配给元件。
...
随着设计需求的变化，您可能需要删除当前的贴花分配，以便可以为元件重新分配不同的贴花。
...
该贴花已从元件上取消分配，并移至“未分配贴花”列表框。

提示：作为快捷方式，双击贴花名称可将其从“已分配贴花”列表中删除。

#### 更改默认贴花

“已分配贴花”列表框中列出的第一个贴花是将元件添加到布局时使用的默认贴花。您可以将“已分配贴花”列表中的任何贴花移动到列表顶部，使其成为默认贴花。
...
使用“门”选项卡将门信息分配给元件，包括门的数量、门交换信息和元件的 CAE 贴花。

注意：空格和句点 (.) 对于引脚名称来说是非法字符。

门贴花和替代门和引脚交换信息将门分配给元件
...
“为浇口指定贴花”对话框用于为元件中的每个浇口指定贴花。某些元件可能包含多个不同类型的浇口，因此您可以为每个浇口指定单独的特定贴花。
...
• 您最多可以指定四个贴花。指定的贴花必须具有相同的引脚数。第一个贴花是默认贴花，即在原理图中添加元件时使用的贴花。

1. 要更改顺序，请在“已分配贴花”列表中选择贴花，然后单击“向上”或“向下”。
...
您可以使用多种方法将引脚添加到元件。您可以通过分配贴花自动添加所有引脚。您可以向元件添加单个引脚、添加一系列引脚以及从数据库粘贴引脚；或者，您可以使用逗号分隔值 (CSV) 文件导入引脚。
...
> [!TIP]
引脚编号可以是数字或字母数字。在 PADS2007 之前的版本中，字母数字引脚编号在 PCB 贴花上不合法，但会覆盖数字贴花编号，并存储在“元件类型”中。您可以继续保留数字 PCB 贴花，并使用“引脚映射”选项卡将不同的引脚编号叠加到数字 PCB 贴花引脚编号上。请参阅“元件信息 - 引脚映射”。
...
• 在“常规”选项卡上，选中“定义元件类型引脚号到 PCB 贴花的映射”复选框，以启用“引脚映射”选项卡。在“PCB 贴花”选项卡上，指定一个带有连续数字引脚号的贴花，即可使用“引脚映射”选项卡。贴花决定了元件中的引脚数。
...
• 在“元件信息”对话框的“常规”选项卡中，选中“定义元件类型引脚号到 PCB 贴花的映射”复选框，以启用“引脚映射”选项卡。在“PCB 贴花”选项卡中，指定一个具有连续数字引脚号的贴花，即可使用“引脚映射”选项卡。该贴花决定了元件中的引脚数。

**程序**

1. 单击“元件信息”对话框中的“引脚映射”选项卡。在预览窗口上方的贴花列表中，选择要映射字母数字引脚的指定贴花。
2. 使用以下方法之一映射引脚：

• 在“未映射的引脚”列表中，选择一个或多个字母数字。如果您要映射的是连续的列表，请选择“元件类型”列中的一项或首行。点击“映射”。
• 在“元件类型”列中选择一个单元格，然后单击“编辑”按钮，或者直接双击该单元格。在“未映射引脚”列表中选择一个字母数字，然后在贴花预览窗口中双击该引脚，即可将该字母数字映射到该引脚。“未映射引脚”列表中的下一行将成为下一个选定的要映射的字母数字。
...
使用“将元件和浇口贴花另存为”对话框将元件类型保存到库中。您还可以在保存过程中重命名关联的贴花。如果您更改贴花中的引脚数或其他信息，这可以防止使用相同贴花的其他元件受到影响。

> [!TIP]

如果减少关联贴花中的引脚数，则“元件属性”对话框的“更改类型”选项将不会使用库中的修改版本更新元件的原理图副本。

**程序**

1. 单击文件 \> 另存为菜单项。
2. 在“将元件和门贴花另存为”对话框中，在元件名称文本框中键入名称。
3. 从库下拉列表框中选择一个库位置。
...
了解元件的位置可以帮助您更好地管理库内容。元件类型和贴花保存在特定位置，以便按逻辑顺序排列，并在需要时找到。

用于元件类型和相关贴花的库文件夹如下所述：

• 元件类型数据将保存在“将元件和门贴花另存为”对话框中选择的名称和库下。
• 编辑元件类型时创建的贴花与元件类型保存在同一个库中。
已修改但未重命名的贴花将保存到打开它们的库中。
重命名的贴花，无论是否修改，如果在所有库中都是唯一的，则保存在与元件类型相同的库中。
...
如果您修改了元件类型并希望保留现有库副本，请使用“将元件和门贴花另存为”对话框重命名该元件。修改电气信息后，您可以保存修改后的元件。

**程序**

1. 单击“文件 \> 另存为菜单项。弹出“将元件和门贴花另存为”对话框。
2. 在元件名称文本框中输入元件类型的新名称。
3. （可选）使用“库”下拉列表框指定新的库位置。另请参见已保存元件类型的库管理、使用其他名称保存修改的贴花。
4. 单击“确定”。
...
引脚贴花用于表示元件上的端子引脚。您可以复制现有引脚贴花并进行修改，也可以创建新的引脚贴花。

从现有别针贴花创建新别针 创建新别针贴花

#### 从现有别针贴花创建新别针

为了节省设计时间并促进重复使用，您可以从现有的大头针贴花创建新的大头针。

**程序**

1. 在元件编辑器中，单击“打开”按钮。
2. 在选择编辑项目类型对话框中，选择“Pin Decal”，然后单击“确定”。显示 Pin Decal 浏览对话框。
3. 从“引脚”区域中选择一个现有的引脚贴花。高亮显示的贴花将显示在“图片”区域中。
4. 单击“确定”。
5. 修改贴花的图形并根据需要定位文本字符串。
...
当您创建新的引脚贴花时，SailWind Logic 会自动创建与引脚信息相关的必要文本字符串。您只需指定代表端子引脚的二维线条，然后重新定位文本字符串即可。
...
提示：要控制在引脚贴花列表中显示哪些贴花，请使用引脚列表管理器

### 在贴花编辑器中编辑对象

贴花编辑器提供了一个灵活的贴花编辑环境。您可以创建新元件、编辑现有元件，还可以基于现有元件创建衍生元件。

要在贴花编辑器中编辑对象，请参阅以下主题：

• 贴花编辑器中的对象选择控制在贴花编辑器中选择多个对象在贴花编辑器中更改对象修改终端
