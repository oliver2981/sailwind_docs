...
您可以双击图钉来跳过一个数字。
如果需要，请使用零件编辑器中编辑工具栏中的“更改引脚号”按钮来更改引脚号。
完成后，右键单击并单击取消。
...
每次单击引脚时，引脚名称都会递增，这样您就可以按顺序为零件分配引脚名称（例如，D0、D1、D2，依此类推）。要跳过序列中的特定引脚名称，请选择相同的引脚，直到出现所需的引脚名称，然后继续选择其他引脚。

1. 完成后，右键单击并单击取消。
2. 使用零件编辑器贴花编辑工具栏中的更改引脚名称按钮来更改单个引脚的名称。
...
<table><thead><tr><th><br>信</th><th><br>代表此引脚类型</th></tr></thead><tbody><tr><td>B</td><td><br>双向引脚</td></tr><tr><td>C</td><td><br>开路集电极销</td></tr><tr><td>G</td><td><br>接地针</td></tr><tr><td>L</td><td><br>负载销</td></tr><tr><td>Ｏ</td><td><br>可绑定源引脚</td></tr><tr><td>P</td><td><br>电源引脚</td></tr><tr><td>Ｓ</td><td><br>源引脚</td></tr><tr><td>T</td><td><br>三态引脚</td></tr><tr><td>U</td><td><br>不明确的</td></tr><tr><td>Z</td><td><br>终结者针</td></tr></tbody></table>
...
要进行浇口交换，请在“浇口”选项卡上为浇口分配一个编号。编号相同的浇口可以在零件内部交换，也可以与其他类似零件交换。数值一 (1) 表示该浇口可与原理图数据库中相同零件类型的浇口交换。如果零件包含多种可交换浇口，则用数字 2 标识第二种类型，用数字 3 标识第三种类型，依此类推。数字 0 表示不能进行交换。
...
提示：示例 CSV 文件位于您的 \\SailWind projects\\Samples 文件夹中。
...
使用“零件信息”对话框中的“属性”选项卡可以为零件添加属性。有大量预定义的属性类型可供选择。
...
贴花编辑器提供了一个灵活的贴花编辑环境。您可以创建新零件、编辑现有零件，还可以基于现有零件创建衍生零件。
