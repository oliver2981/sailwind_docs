...
• 点击相应按钮，放大所选图页或图页范围。• 在预览窗口中单击鼠标左键，可在光标位置放大。单击鼠标右键，可缩小。
...
1. 在“创建 PDF”对话框的“图页至 PDF”列表中，选择您不想包含在 PDF 中的原理图页，然后单击“删除”。
...
1. 为了能够查看 PDF 中的元件属性（例如参考指示器和元件类型），请选中“创建将显示元件属性的超链接”复选框。
...
1. 要在元件、网络名称和总线上创建不带黄色和蓝色框的超链接，请清除“带超链接的对象周围的可见矩形”复选框。
...
• 摘要区域列出了您可以打印的可用编号图页以及图页中包含的项目。• 文件前缀框列出了要打印的文件的前缀名称。
