...
PCB 的特定制造配置。装配变体指定了哪些组件可用、哪些组件不可用以及哪些组件可用其他封装部件类型替换。单个 PCB 可以存在多个装配变体。
...
此命令在多个“簇放置”层级上均有效。选中后，它会清除电路板上所有未粘贴的部件或簇，并根据封装类型将它们排列在电路板轮廓的外部。
