...
> [!TIP]
使用过滤器查找元件。使用通配符约定（带或不带前导字符）来扩展或缩小过滤器范围。单击“应用”以激活过滤器。

1. 单击“添加”。该元件将附加到光标并跟随光标移动。

（可选）您可以旋转或镜像项目，然后再指定其位置。右键单击，然后根据需要多次单击“旋转 90 度”、“X 镜像”和“Y 镜像”，以设置项目的正确方向。单击“替代”以放置替代符号（如果可用）。单击“下一个类型”以选择元件中的其他门（如果可用）。

1. 单击原理图来放置元件；另一个实例会自动附加到光标。
2. 添加完元件后，右键单击并单击取消弹出菜单项或按 Esc 键。
...
连接器 Pin 是 SailWind Logic 库中的特殊符号。您可以像添加多门元件一样添加它们，每次添加连接器符号时， Pin 编号都会递增。只有当您使用完所有连接器 Pin 后，SailWind Logic 才会递增参考标识符。

添加连接器的一个或多个 Pin 并建立参考标识符后，使用“使用复制模式”添加其他 Pin 。这会使 SailWind Logic 对新 Pin 使用相同的参考标识符。
...
添加新的多门元件时，将使用 Pin 号最低的门，以及下一个可用的参考标识符和字母数字后缀（或门修饰符）。参考标识符和后缀之间用短划线 $(-)$ 分隔。例如，添加到新原理图中的 7400 将使用 Pin  1 至 3 的门，并将其指定为元件 U1-A。添加同一元件的其他门时，将使用下一个可用的门和门修饰符，例如，将添加 Pin  4 至 6 的门并将其指定为 U1-B。
...
此门 — 更新选定的门。此元件 — 更新某个元件或该元件的所有关联门。• 所有元件 此类型 — 更新设计中所有匹配的门或元件
...
• 如果库中已存在您正在保存的元件类型或逻辑封装，则会出现一个消息框。单击“是”可覆盖库中的元件类型或逻辑封装。单击“否”可仅保存库中尚不存在的元件类型。单击“取消”可返回“将元件保存到库”对话框，而不将元件类型保存到库中。• 当同类型的元件具有相同值的属性时，属性名称和值将与元件类型一起保存在库中。当同类型的元件的相同属性名称具有不同的值时，只有值为空的属性名称会保存到库中。
...
元件类型 CAE 封装  Pin 封装 接地符号 电源符号 页间连接符

您可以比较或更新：

• 以上任何或所有类别的全部。
• 选定的元件类型、CAE 封装和 Pin 封装。您无法比较/更新选定的接地、电源或页间连接符。

**警告：**

由于库和原理图内容之间的唯一链接是条目的名称，因此更新可能会产生无法预料的意外后果。当在原理图中全局替换元件类型或门时，它们的替换与条目的本地编辑无关；已编辑的元件或门实例将被库中的通用实例替换。此类更改的不良后果可能很小，例如标签重新定位，也可能非常严重，例如 Pin 被删除或网络断开。
...
比较/更新过程 如何阅读更新报告 从库更新对话框 从库中更新选定的元件类型 从库中更新选定的 CAE 封装 从库中更新选定的 Pin 封装
...
比较/更新过程 如何阅读更新报告 从库中更新原理图 从库中更新选定的 CAE 封装 从库中更新选定的 Pin 封装
...
此过程会更新 CAE 封装中的 Pin 封装分配，但不会更新 Pin 封装本身。请使用“从库中更新选定 Pin 封装”中的步骤之一来更新 Pin 封装。

因此，例如，如果您将 CAE 封装 X 更新为使用  Pin B 而不是  Pin A，它将使用原理图中当前  Pin B 几何体的版本。如果原理图没有 PINB，则会从库中安装。
...
2. 根据需要修改“从库中更新选定的 CAE 封装”对话框中的设置。
3. 单击“确定”。
...
您可以使用“从库更新”对话框或直接从原理图编辑器中更新库中选定的 Pin 封装。

使用“从库更新”对话框更新选定的 Pin 封装 使用原理图编辑器更新选定的 Pin 封装

#### 使用“从库更新”对话框更新选定的 Pin 封装

当您对库中的内容进行更改和更新时，可能需要将设计中的 Pin 封装与更新后的库内容同步。您可以使用“从库更新”对话框来更新一个或多个选定 Pin 封装的所有实例。
...
2. 设置“从库更新”在库中搜索与设计中的项目匹配的项目时使用的库列表顺序。（请参阅“常规比较/更新规则”。）单击“确定”，然后关闭“库管理器”对话框。
3. 单击“工具 \> 从库更新菜单项。
4. 选中 Pin Decals 复选框，然后选择要比较/更新的封装。
5. 单击“确定”。
...
#### 使用原理图编辑器更新选定的 Pin 封装

当您更改库中的内容时，可能需要刷新设计内容以反映这些更新。您可以使用“从库中更新选定 Pin 封装”对话框选择一个或多个 Pin ，并更新选定 Pin 所使用的 Pin 封装的所有实例。

**程序**

1. 单击文件 \> 库菜单项，然后单击管理库列表按钮。
2. 设置“从库更新”在库中搜索与设计中的项目匹配的项目时使用的库列表顺序。（请参阅“常规比较/更新规则”。）单击“确定”，然后关闭“库管理器”对话框。
3. 在原理图编辑器中，不选择任何内容，右键单击并单击选择 Pin 弹出菜单项。
4. 选择您想要比较/更新的 Pin 。

0 提示如果您想更新除少数针脚封装之外的所有针脚封装，请选择所有元件/门，然后在步骤 6 中清除您不想更新的封装的复选框。

1. 右键单击并单击更新 \> Pin Decal 菜单项。
2. 根据需要修改“从库中更新选定 Pin 封装”对话框中的设置。
3. 单击“确定”。
...
When an item in the design is newer than the equivalent item in the library, and you reply No to the “当设计中的某个项目比库中的等效项目更新时，您会对“原理图中的时间戳比库中的时间戳更新。确定覆盖原理图中的版本吗？”提示回答“否”。
• 当库中较新的项目的 Pin 数比设计中等效较旧的项目的 Pin 数少时。
• 当库中的 GND、POWER 或 OFF 符号数量少于设计中可用符号的数量时。

**旧原理图**

2005 SPac2 之前版本的 PADS 原理图没有时间戳。打开这些原理图时，所有元件类型、封装、 Pin 封装以及页间连接符、电源和接地符号上的时间戳均会设置为零。要确定原理图中元件类型和封装的内容是否与库同步，请选中“隐藏相同结果”复选框，并比较原理图和库。
...
| 元件类型 | 逻辑系列、门数量、连接器 Pin 数量、信号 Pin 数量、属性名称列表 |  |
|  | 对于每个门： | CAE 封装数量、CAE 封装名称、门上的电气 Pin 数量、门交换类别。 |
|  | 对于每个门 Pin ： |  Pin 编号（如果已定义，则为字母数字）、 Pin 名称、 Pin 类型、 Pin 交换类 |
|  | 对于每个信号 Pin ： |  Pin 编号（如果定义则为字母数字）、网络名称 |
...
| PCB 封装针数 |  |  |
|  | 对于连接器：对于普通元件： | PCB 封装中分配给每个连接器元件的 Pin 数等于为该元件类型定义的连接器 Pin 数。PCB 封装中分配给每个普通元件的 Pin 数为 |
|  |  | 等于或大于库元件类型定义的门电路中使用的最高电气 Pin 数。限制： Pin 数无法在原理图中更新。 Pin 数差异将在更新报告中标记为警告。 |
| CAE 封装 |  |  |
|  | 门封装详细信息： | 属性标签数量、属性标签名称、文本数量、端子数量。 |
| 限制：· 不比较 CAE Decal 图形。 |  |  |
|  | 终端详情： | · 文本项的详细信息未进行比较。 Pin 封装名称已分配给每个端子。 |
|  |  | 限制：如果端子 Pin 数不同，则无法在原理图中单独更新封装，而不受元件类型影响。 |
...
| --- | --- | --- |
|  Pin 封装 |  | 固定封装名称。 |
| 更新说明：· 因为默认 Pin 名称和 Pin 编号标签位置在创建 CAE 封装时是“硬连线”的，所以 |  |  |
| 当 Pin 封装更新时，原理图中的 Pin 名称和编号不受影响。·  Pin 封装确实为原理图上的网络名称提供了默认位置，但实际使用的网络名称位置是单独存储的，因此网络名称位置不受更新的影响。 |  |  |
| 电源和接地符号 |  | 符号名称、类型、信号 Pin 名称 |
| 页间连接符 |  | 符号名称、类型 |
...
报告摘要 — 以表格形式列出比较的元件类型、CAE 封装、页间连接符、接地符号和电源符号，并显示整个软件包的比较结果和更新状态。示例：
...
| 无法从库获取 PIN DECAL！！！ | 细节 |  Pin 封装 | 针脚封装存在于设计中但不在库中。 |
...
| 信息 | 报告部分 | 参考： | 方法 |
| --- | --- | --- | --- |
| 不同的时间戳 | 概括 |  Pin 封装时间戳 | 设计和库中的时间戳不同。 |
...
| 未找到或未选择  Pin  封装！！！ | 细节 |  Pin 封装 | 或者：· 在设计中未找到对话框中选择的 Pin 封装，或者· 选择了 Pin 封装复选框，但未选择 Pin 。 |
| 未分配 | 细节 | 栅极 Pin  | 设计中分配的门极 Pin 未在库中分配。 |
...
| WARNING | 细节 | PCB 封装 | 库中的 PCB 封装的 Pin 比设计中的少。 |
...
4. 要保留删除元件或总线后剩下的单 Pin 网络，请右键单击并单击“保留连接”弹出菜单项。
...
1. 要保留删除元件或总线后剩下的单 Pin 网络，请右键单击并单击“保留连接”弹出菜单选项。
...
将文件中的组粘贴到您的设计中时，组中未连接的 Pin 、未端接的电线或总线段可以自动连接。

• 未连接的 Pin 自动与未端接的电线末端或未连接的 Pin 连接。• 仅当未端接的电线是总线的命名子网时，未端接的电线末端才会自动连接到总线段。
...
• 要为库中的所有元件添加属性，请在“管理库属性”对话框中创建属性。另请参阅“管理库属性”。要为打开的设计中的每个元件添加属性，请在“管理原理图属性”对话框中创建属性。另请参阅“管理原理图中的属性”。
• 要为特定元件添加属性，请在编辑该元件时使用“属性”选项卡创建属性。另请参阅“管理属性”。要为原理图上的元件添加属性而不更新元件类型，请使用“元件属性”对话框。

另请参阅修改元件属性和元件上定义的电阻值。

要在原理图中显示创建后的属性，请使用可见性控件。另请参阅控制元件的文本可见性。
