...
此门 — 更新选定的门。此部件 — 更新某个部件或该部件的所有关联门。• 所有部件 此类型 — 更新设计中所有匹配的门或部件
...
| PCB 封装针数 |  |  |
|  | 对于连接器：对于普通部件： | PCB 封装中分配给每个连接器部件的引脚数等于为该部件类型定义的连接器引脚数。PCB 封装中分配给每个普通部件的引脚数为 |
...
| 未在库部分分配 | 细节 | 比较商品 | 设计中分配给该部件的 PCB 封装不在库中分配给该部件的封装集中。 |
