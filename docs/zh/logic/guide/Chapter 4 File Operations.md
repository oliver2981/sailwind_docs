...
随后，每个工作表的所有数据都会分组到 SHT 部分中。这些数据以 CAE 开头，列出了工作表的窗口缩放设置和光标位置。
...
| TXT | 工作表中的自由文本项目。 |
| LINES | 2D 行项目，包括库条目。 |
| CAEDECAL | 图纸中所有元件的 CAE 符号封装的描述。 |
| PARTTYPE | 工作表中出现的元件类型的描述。 |
| BUSSES | 表格中Bus的描述，包括名称和位置。 |
| PART | 列出图纸中出现的所有元件及其属性。门和​​连接器 Pin 均单独列出。 |
| OFFPAGE REFS | 列出图纸中所有页外标记。其中包括电源和接地符号以及总线连接。 |
