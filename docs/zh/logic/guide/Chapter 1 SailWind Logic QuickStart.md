...
2. 在“从库添加元件”对话框的“过滤器”部分中，将库设置为“所有库”。

3. 在项目框中，键入 \* 通配符，然后单击应用。

4. 选择一个元件，并在预览窗口中查看该元件的符号。点击“添加”。轮廓形状的元件将附着在指针上，并准备放置。
...
连接器的每个 Pin 在原理图中都有一个唯一的符号。因此，多个符号共同构成一个元件。所有连接器 Pin 都具有相同的参考标识符，以表明它们是同一元件的符号 Pin 。

**步骤**

1. 在逻辑示意图编辑工具栏上，单击添加部分按钮。
2. 在“从库中添加元件”对话框的过滤器区域中，从库列表中选择“连接”库，然后单击“应用”。
...
2. 在组件 Pin 的端点处，单击以开始连接并绘制连接。默认情况下，连接是正交的，您必须单击才能创建角。
...
当设计中需要连接到另一张原理图图纸上的电路时，需要使用页间连接符。页间连接符有多种表示形式，类似于连接器。

**步骤**

1. 从设计中某个元件的 Pin 创建连接。
