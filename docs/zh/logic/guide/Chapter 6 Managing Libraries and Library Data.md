...
| 文件扩展名 | 文件内容 |
| --- | --- |
| .br | 部件——有关部件（例如 74LSo2）的数据，包括逻辑系列、属性、引脚和门。 |
| .pd | 贴花 - 绘制元件时的图形表示。通常称为封装。 |
| .ld | Logic_—— 原理图元件的图形表示，例如或非门。此部分仅用作元件清单读取器。使用 SailWind Logic 创建和修改 Logic 贴图（有时称为 CAE 贴图）。 |
| 。在 | 线条——您可以存储在库中的一般图形数据，例如公司徽标，以便在任何设计文件中使用。 |
...
您可以在“库管理器”对话框中显示库中现有的项目。您可以选择查看 PCB 贴图、元件类型、绘图项目或逻辑 CAE 贴图。
...
库项目名称显示在 PCB 贴花、元件类型、线项目或 CAE 贴花列表中。（列表名称会根据您选择的过滤器而变化。）预览窗口会显示库对象的图形。

> [!INFO]
由于库元件没有视觉表示，预览窗口显示与该元件关联的第一个 CAE 贴花。
...
元件 — 您将进入元件编辑器。单击“编辑电气”按钮。系统将显示“元件信息”对话框，您可以在其中定义新元件。另请参阅修改元件的电气信息。
...
1. 在“PCB 贴花”、“元件类型”、“行项目”或“CAE 贴花”列表中选择一个或多个项目。（列表名称会根据您选择的过滤器而变化。）
...
限制：如果选择“（所有库）”，则“复制”按钮不可用。

1. 单击以下按钮之一可显示库中的项目类别：

• 贴花 — PCB 贴花（元件封装）元件 — 元件线 — 绘图对象
• 逻辑——CAE 贴花（示意图符号）
...
• 贴花 — 打开将 PCB 贴花保存到库对话框 • 元件 — 打开将元件类型保存到库对话框 线条 — 打开将绘图项目保存到库对话框 • 逻辑 — 打开将 CAE 贴花保存到库对话框
...
• 元件 — 进入元件编辑器。点击“编辑电气”按钮。此时会弹出“元件信息”对话框，您可以编辑元件。请参阅“修改元件的电气信息”和“元件编辑器操作”。
...
您可以将库添加到库列表中，以使其内容可用。库必须已在“库管理器”中列出，才能搜索并在设计中使用其元件、贴花或线项。
...
您创建的元件、符号和贴花不必位于同一个库中。当您的元件引用其他库中的符号或贴花时，软件会在需要时自动拾取该符号或贴花。
...
使用“管理库属性”对话框可以逐个库地管理属性。您可以添加、删除和重命名单个库或所有库中所有元件或贴花的属性。您还可以显示库中的所有属性，无论这些属性是应用于所有项目还是应用于单个项目。
...
您可以向一个或所有库中的所有元件和贴花添加属性，或者仅向单个元件或贴花单独添加属性。
...
3. 在项目类型列表中，选择是否将新属性应用于所有项目，或者仅应用于元件类型或 PCB 贴花。
...
新属性已添加。请在“贴花属性”对话框（针对 PCB 贴花）或位于第 631 页的“元件信息”对话框的“属性”选项卡（针对元件类型）中检查新属性。
...
您可以从一个或所有库中的所有元件和贴花中删除一个或多个属性，或者仅从单个元件或贴花中删除一个或多个属性。
...
1. 单击文件 > 库菜单项，然后在库管理器对话框中单击属性管理器按钮。
2. 在“管理库属性”对话框的“选择库”列表中，选择单个库或列表顶部的“（所有库）”项。
3. 在项目类型列表中，选择是否从所有项目中删除属性，或者从元件类型或 PCB 贴花中删除属性。
...
您的属性已被删除。请在“贴花属性”对话框（针对 PCB 贴花）或位于第 631 页的“元件信息”对话框的“属性”选项卡（针对元件类型）中检查已删除的属性。
...
您可以重命名一个或所有库中的所有元件和贴花的属性，或者仅重命名单个元件或贴花的属性。
...
1. 单击文件 > 库菜单项，然后在库管理器对话框中单击属性管理器按钮。
2. 在“管理库属性”对话框的“选择库”列表中，选择单个库或列表顶部的“（所有库）”项。
3. 在项目类型列表中，选择是否从所有项目、元件类型或 PCB 贴花中重命名属性。
...
您的属性已重命名。请在“贴花属性”对话框（针对 PCB 贴花）或位于第 631 页的“元件信息”对话框的“属性”选项卡（针对元件类型）中检查重命名的属性。
...
• 如果文件类型为.p，请选择“元件”过滤器。此 ASCII 文件包含元件类型。
...
3. 在“过滤器”区域中，单击“元件”。库中（或所有库中）的元件列表将显示在“元件类型”区域中。

> [!TIP]
要优化列表，请使用筛选字段。在字段中输入元件名称，或使用通配符 $(^{\star})$ 指定一组元件。然后点击“应用”。

1. 当您拥有要报告的元件列表时，请单击“列出到文件”。
2. 在报告管理器对话框中，指定要包含在报告中的元件属性。
...
• 库列表格式 $(.I s t)$ ：信息按列格式化，以便查看或打印。（图 1）。
...
1. 在过滤器区域中，单击贴花、线条或逻辑过滤器。

对话框中会出现库中相应项目的列表（在“PCB 贴花”、“线条项目”或“CAE 贴花”区域中，取决于您的过滤器选择）。

> [!TIP]
要选择一个或多个特定项目，请使用筛选字段。在字段中输入项目名称，或使用通配符 $(^{\star})$ 指定一组项目。然后点击“应用”。

1. 当您有了想要报告的列表时，请单击“列出到文件”。
2. 在“库列表文件”对话框中，指定报告的文件夹和文件名，然后单击“保存”。
...
该过滤器支持表 13 中列出的通配符和表达式。表 14 给出了通配符使用的示例。
...
| 一 | 精确匹配特殊语法字符，并隐藏该特殊字符的语法意义。提示：以下字符前面需要加 \： [ ?!^-' |
...
| 74A[通道]* | 以 74A 开头，后跟 C 或 H，并以任意数量的字符结尾：74AC244,74AHCT27。 |
| 74A[!CH]* | 以 74A 开头，后跟除字母 C 到 H 之外的任意字符，并以任意数量的字符结尾：74ABT44、74ALS244、74ABF365。 |
| [M]*08 | 以字符开头，后跟任意数量的字符，并以 08 结尾：\LS08,\HCT08,\ABT08。 |
...
• 使用各种对话框搜索库项目 • 将原理图网表导入 SailWind Layout • 从库更新设计 • 使用原理图上的新元件注释 SailWind Layout 您可以使用“库列表”对话框更改库的搜索顺序。另请参阅“设置库列表顺序”。
