...
要添加绘图对象（例如多边形、圆形、矩形、路径以及圆弧），请点击“逻辑示意图编辑”工具栏上的“创建2D线”按钮。此操作会将默认弹出菜单更改为用于创建2D线项的特定弹出菜单。
...
您可以使用“修改2D线”按钮来更改绘图对象。这会将默认弹出菜单更改为用于修改2D线项目的特定弹出菜单。
...
使用弹出菜单可启用其他2D线修改。如果选择一条线段，则可以使用以下修改功能：
...
• 组合只能包含2D线条或文本。这与第 215 页的组不同，组是原理图上对象的集合（可能包含或不包含2D线条或文本）。

**步骤**

使用下列方法之一：

• 使用对象模式 a. 选择2D线条或文本块。b. 按住 Ctrl 键并单击，继续选择要合并的项目。c. 右键单击​​，然后单击“合并”弹出菜单项。选定的项目将被合并。

提示：当您从组合中选择2D线条项进行移动、复制等操作时，所有线条项都会被选中进行操作。先选择文本不会选中整个组。
...
> [!TIP]
合并2D线条和文本时，请务必先选择2D线条，然后再选择文本。如果您在选择2D线条之前尝试选择文本项，则会显示“没有可添加文本的绘图项”消息。
...
• 使用对象模式：a. 从组中选择一个2D线条项目。b. 右键单击​​，然后单击“分解”。组合不再组合，对象分离。
...
6. 您可以选择覆盖该项目或将其另存为新项目。单击“确定”会将2D线对象保存回2D线库。
