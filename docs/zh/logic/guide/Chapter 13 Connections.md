...
连接是设计的核心，它使您能够捕捉并传达设计的互连性。SailWind Logic 提供了一系列强大的命令，可用于添加、命名和连接网络。您可以完全连接设计对象，或使用浮动连接作为占位符，用于未来的设计决策。您可以添加、移动分割网络或段。您还可以根据需要交换元件上的 Pin 。此外，还有一组强大的命令可用于创建、编辑和管理总线。

## 添加连接

添加连接时，必须具有电气上有意义的起点和终点。连接可以从符号 Pin 或其他连接开始。连接可以从符号 Pin 、其他连接、总线、页外引用或接地或电源符号结束。除非启用“浮动连接”，否则 SailWind Logic 会阻止您在数据库中的任意位置开始或结束连接。
...
1. 要为连接分配固定网络名称，必须先添加连接，然后选择直接连接到元件类型 Pin 的线段。然后，选择该网络，右键单击，单击“属性”，并在“网络属性”（第 289 页）对话框中输入新的网络名称。
...
| HyperLynx.Default IC.Model Pin | 自由文本 | 列出上述属性中模型的特定 Pin （如适用）。用于 BoardSim 仿真。|
| PowerGround | 是/否 | 值：文本（ Pin 名称或编号）将网络标识为地线和电源网络。|
表 30. 网络默认属性列表（续）
...
浮动连接为您的原理图设计增添了更高级别的灵活性。启用浮动连接后，您可以添加从 Pin 到尚不存在元件位置的连接，而无需使用页间连接符。您还可以在空白处开始连接，稍后再进行连接。未连接的网络将以方形标记终止。
...
警告：禁用浮动连接并不会将其从设计中移除。禁用此首选项仅可防止创建额外的浮动连接。完成的原理图上不需要浮动连接。浮动端应连接到 Pin 或终止于页间连接符。运行连接报告以查找设计中浮动连接的位置。
3. 单击文件$\>$报告菜单项。
...
启用“浮动连接”首选项后，您可以将连接悬空在空间中，以便稍后进行连接。您还可以在空间中启动连接，以备稍后连接。通过 ECO 导入自动添加的新元件和连接短截线将接收未连接的标记，而不是页间连接符。通过 ECO 导入添加到现有元件的网络短截线将继续接收页间连接符。

创建悬空连接 创建浮动连接

#### 创建悬空连接

悬垂连接仅在一端与设计对象绑定，以预期在设计过程的未来某个时间点将其连接到另一端。
...
启用“浮动连接”首选项后，您可以复制连接段并将其粘贴到空间中。无需将它们粘贴到 Pin 到 Pin 的连接位置。
...
启用“浮动连接”首选项后，已删除元件的连接将以“未连接标记”而非“页间连接符”终止。删除总线时，连接到该总线的网络也将以“未连接标记”而非“页间连接符”终止。

### 将对象附加到浮动连接

启用“浮动连接”首选项后，任何接触浮动连接端的组件 Pin 都会自动连接。接触其他组件 Pin 的 Pin 也会自动连接。将组件移开即可使连接可见。

### 添加页外引用

要连接原理图不同图纸上或同一张图纸两端的两个 Pin （无需图形连接），请使用具有相同网络名称的页外参考。
...
带有右侧 Pin 的默认封装定义为源 Pin ；带有左侧 Pin 的替代封装定义为负载 Pin 。此 Pin 类型方向在创建图纸的分层符号时会发挥作用。所有页外源均位于分层符号轮廓的左侧，所有负载均位于右侧。您可以放置​​默认封装，也可以右键单击并单击“替代”。

（可选）您可以使用适当的命令旋转或镜像符号。

1. 找到光标并双击以结束连接或右键单击并单击完成
2. 在对话框中输入网络名称，然后单击“确定”。

如果在“选项”对话框的“设计”类别中选择了“显示页外图纸编号”选项，则连接符号的图纸编号将显示在页间连接符旁边。
...
要交换两个元件的 Pin ，您可以使用原理图编辑工具栏中的“交换 Pin ”按钮或弹出菜单中的“交换 Pin ”。SailWind Logic 会交换您选择的 Pin 的 Pin 编号、名称和端子。您还可以在放置元件时使用弹出菜单中的“镜像 X”和“镜像 Y”来交换 Pin 。这通常有助于改善原理图的流程。

使用动词模式下的“交换 Pin ”按钮交换 Pin  通过一次选择一个 Pin 来交换 Pin  通过首先选择两个 Pin 来交换 Pin 

### 使用动词模式下的“交换 Pin ”按钮交换 Pin 

您可以使用“交换 Pin ”按钮交换 Pin 。使用此方法，您可以先启动命令，然后选择要交换的 Pin 。您可以继续选择要交换的 Pin 对，直到取消命令为止。

**步骤**

1. 在逻辑示意图编辑工具栏上，单击交换 Pin 按钮。
2. 选择第一个密码。
3. 选择要与第一个选定的 Pin 交换的 Pin 号。
4. 如果您选择的 Pin 类型不匹配，则会显示消息“警告， Pin 交换类型不匹配...”
出现消息。单击“是”继续。单击“否”取消交换。

> [!TIP]
还可能出现“是否将连接绑定到同一电气 Pin ？”消息。单击“是”可保留与 Pin 编号相同的连接。单击“否”可重新连接到重新编号的 Pin 。

要使用弹出菜单交换 Pin ，您可以一次选择一个 Pin ，也可以同时选择两个 Pin ：

### 通过一次选择一个 Pin 来交换 Pin 

您可以通过一次选择一个 Pin 来交换 Pin 。此方法允许您选择第一个 Pin ，启动命令，然后选择第二个 Pin 。要创建另一个 Pin 交换，您需要重复此过程。
...
2. 右键单击并单击“交换 Pin ”。
3. 选择另一个密码来完成交换。
4. 如果您选择的 Pin 类型不匹配，则会显示“警告， Pin 交换类型不匹配…”消息。点击“是”继续。点击“否”取消交换。
...
您可以通过先选中两个 Pin 来交换 Pin 。选中两个 Pin 后，即可启动命令。要创建另一个 Pin 交换，请重复此过程。

**步骤**

1. 使用 Ctrl+单击，选择要交换的 Pin 。
2. 右键单击，然后点击“交换 Pin ”。如果出现“交换已连接 Pin ？”消息，请点击“是”继续。点击“否”取消交换。
3. 如果您选择的 Pin 类型不匹配，则会显示“警告， Pin 交换类型不匹配…”消息。点击“是”继续。点击“否”取消交换。
...
如果删除操作将总线拆分为两条独立的总线，则会显示总线名称，以标识任何未命名的总线段。每条总线均保留相同的属性。
...
所有网络均已分离。根据“浮动连接”首选项，网络将被赋予未连接的标记或页间连接符。
