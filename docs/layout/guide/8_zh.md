...
6. 使用以下方法之一映射管脚：
    ...
    - **使用预览窗口** 在未映射管脚列表中选择一个字母数字，然后双击封装预览窗口中的管脚以将字母数字映射到管脚。未映射管脚列表中的下一行成为下一个要映射的选定字母数字。
      ...
      > [!tip] 在预览窗口中，您可以单击并拖动以定义缩放框，或使用 Shift+单击或 Shift+右键单击以按两倍放大或缩小。您可以放大到原始比例的 16 倍。预览窗口只会缩小以适应封装完全显示在视图中。
...
3. 在"将元件类型和封装保存到库"对话框中，选择元件类型和封装列表中的项目。
   ...
   > [!tip]    如果您创建了一个新的后缀封装，意味着您仅更改了一个组件的焊盘堆栈，并且封装名称具有唯一后缀，则新封装也会出现在封装列表中。
