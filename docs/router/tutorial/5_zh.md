...
> [!tip] 此时 DRC 模式关闭。新走线段不禁止与其他对象短路。如果启用了 DRC，此时可通过输入 **DRC** 然后按 **Enter** 禁用它。
...
在具有多个连接的网络中，连接的端点会自动捕捉到网络上最近的管脚。通过将指针移向电路板的左下角来尝试重新连接。一旦走线段的端点更接近 U7，它就会从当前端点分离并捕捉到 U7。将指针移向左上方，走线段会重新捕捉回来。此功能让您在布线时动态重新连接走线，无需手动重新排序连接。
...
> [!tip] 如果启用了 DRC，当最后一个走线拐角位于元件焊盘内时，不会添加过孔。
...
> [!tip] 要在启用动态时备份走线，慢慢地在新建的走线模式上后退。
...
2. 创建新的走线模式并通过点击走线或走线段上的任何其他点完成它。否则，在重新布线期间在元件管脚和过孔上完成时使用正常的走线完成命令
...
6. 现在将走线移离禁布区边框并再次尝试插入过孔。注意过孔在禁布区外如何被允许
