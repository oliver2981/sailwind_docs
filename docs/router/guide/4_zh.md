...
文件[类型](#page-1-1) 打开文件时的[文本字体](#page-1-2) 打开文件时的[差分对](#page-2-0)走线宽度设置 打开文件时的[安全层数检查](#page-2-1) 打开文件时的[T型节点](#page-3-0) 打开文件时的[子网冲突](#page-3-1)
...
有关按层设置宽度和间距的信息，请参阅第184页的"设置差分对属性"。
...
若不超过两个引脚对共享走线段，SailWind Router 会自动建立规则边界使设计符合程序规则。[第42页的"规则边界"](#page-3-1)改进了此类冲突管理，消除了打开文件时需断开走线的要求。
...
有关打开文件的更多详细信息及打开过程中发生的处理，请参阅[第40页的"关于打开文件"](#page-1-0)。
...
3. 在备份数量框中输入要保留的最大备份数量
   ...
   **提示** 此限制可指定所有设计的最大备份数量，或每个设计的备份数量。更多信息请参阅第438页的表"表88"。
...
备份将根据本流程中的设置保存，可保存在设计目录或"选项对话框全局类别文件位置子类别"第439页指定的默认备份位置。同时，已保存备份的链接将记录在输出窗口**状态**选项卡显示的会话日志中。
...
3. 选择要归档的文件和文件夹（参见第389页的"归档器对话框"）
...
其中 YYYY 为年，MM 为月，DD 为日，HH 为小时（24小时制），MM 为分钟，SS 为创建文件时的秒数。
